...
// #[requires(...)] and #[ensures(...)] annotations based on type signatures,
// and optionally writes them inline or outputs a .trust.toml config file.
//
...
fn generate_param_preconditions(
    fn_line: &str,
    param_names: &[String],
    ...
    for (name, ty) in &params_str {
        ...
        if is_index_param(name) && is_unsigned_type(ty) {
            ...
            for (other_name, other_ty) in &params_str {
                if (other_ty.contains("&[") || other_ty.contains("Vec<"))
                    && other_name != name
                {
...
///
/// For each annotation, inserts `#[requires(...)]` and `#[ensures(...)]`
/// lines immediately before the function declaration. Processes files in
/// reverse line order to preserve line numbers.
pub(crate) fn write_inline_annotations(annotations: &[ScaffoldedAnnotation]) -> Result<usize, String> {
    ...
    for (file, mut anns) in by_file {
        ...
        anns.sort_by(|a, b| b.line.cmp(&a.line));
        ...
        for ann in &anns {
            ...
            for ensures in ann.ensures.iter().rev() {
                new_lines.push(format!("{indent}#[ensures({ensures})]"));
            }
            for requires in ann.requires.iter().rev() {
                new_lines.push(format!("{indent}#[requires({requires})]"));
            }
            // Reverse so requires come first, then ensures
            new_lines.reverse();
...
mod tests {
    ...
    fn test_generate_annotations_slice_param() {
        ...
        std::fs::write(
            &file,
            "pub fn search(data: &[i32], target: i32) -> Option<usize> {}\n",
        )
        .unwrap();
    ...
    fn test_generate_annotations_unsafe_fn() {
        ...
        std::fs::write(
            &file,
            "pub unsafe fn deref(ptr: *const u8) -> u8 { *ptr }\n",
        )
        .unwrap();
    ...
    fn test_write_inline_annotations() {
        ...
        assert!(
            content.contains("#[requires(true)]"),
            "expected requires annotation, got:\n{content}"
        );
        assert!(
            content.contains("#[ensures(result == a + b)]"),
...
"#,
    ...
    fn test_index_bounds_annotation() {
        ...
        std::fs::write(
            &file,
            "pub fn get(data: &[i32], index: usize) -> i32 { data[index] }\n",
        )
        .unwrap();
