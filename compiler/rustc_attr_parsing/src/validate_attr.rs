...
pub fn check_attr(psess: &ParseSess, attr: &Attribute) {
    ...
    match builtin_attr_info {
        ...
        _ => {
            ...
            if let AttrArgs::Eq { .. } = attr_item.args.unparsed_ref().expect("invariant: normal attr item has unparsed args") { // tRust: unwrap -> expect
...
pub fn parse_meta<'a>(psess: &'a ParseSess, attr: &Attribute) -> PResult<'a, MetaItem> {
    ...
    Ok(MetaItem {
        ...
        kind: match &item.args.unparsed_ref().expect("invariant: normal attr item has unparsed args") { // tRust: unwrap -> expect
