//! tRust: Codegen support for `#[naked]` functions that emit global assembly rather
//! tRust: than a normal function body.
...
fn prefix_and_suffix<'tcx>(
        ...
        match item_data.linkage {
            ...
            Linkage::LinkOnceAny | Linkage::LinkOnceODR | Linkage::WeakAny | Linkage::WeakODR => {
                match asm_binary_format {
                    ...
                    BinaryFormat::Xcoff => {
                        // NOTE: Weak symbol definition not supported in inline assembly.
            ...
            Linkage::ExternalWeak => {
                // NOTE: This path causes SIGILL in LLVM; alignment is applied as workaround.
    ...
    match asm_binary_format {
        BinaryFormat::Elf => {
            ...
            writeln!(begin, ".pushsection {section},\"ax\", {progbits}").expect("write to String buffer is infallible");
            writeln!(begin, ".balign {align_bytes}").expect("write to String buffer is infallible");
            write_linkage(&mut begin).expect("write to String buffer is infallible");
            match visibility {
                Visibility::Default => {}
                Visibility::Protected => writeln!(begin, ".protected {asm_name}").expect("write to String buffer is infallible"),
                Visibility::Hidden => writeln!(begin, ".hidden {asm_name}").expect("write to String buffer is infallible"),
            }
            writeln!(begin, ".type {asm_name}, {function}").expect("write to String buffer is infallible");
            if !arch_prefix.is_empty() {
                writeln!(begin, "{}", arch_prefix).expect("write to String buffer is infallible");
            }
            writeln!(begin, "{asm_name}:").expect("write to String buffer is infallible");

            writeln!(end).expect("write to String buffer is infallible");
            ...
            writeln!(end, ".Lfunc_end_{asm_name}:").expect("write to String buffer is infallible");
            writeln!(end, ".size {asm_name}, . - {asm_name}").expect("write to String buffer is infallible");
            writeln!(end, ".popsection").expect("write to String buffer is infallible");
            if !arch_suffix.is_empty() {
                writeln!(end, "{}", arch_suffix).expect("write to String buffer is infallible");
            }
        }
        BinaryFormat::MachO => {
            let section = link_section.unwrap_or_else(|| "__TEXT,__text".to_string());
            writeln!(begin, ".pushsection {},regular,pure_instructions", section).expect("write to String buffer is infallible");
            writeln!(begin, ".balign {align_bytes}").expect("write to String buffer is infallible");
            write_linkage(&mut begin).expect("write to String buffer is infallible");
            match visibility {
                Visibility::Default | Visibility::Protected => {}
                Visibility::Hidden => writeln!(begin, ".private_extern {asm_name}").expect("write to String buffer is infallible"),
            }
            writeln!(begin, "{asm_name}:").expect("write to String buffer is infallible");
        ...
        BinaryFormat::Coff => {
            let section = link_section.unwrap_or_else(|| format!(".text.{asm_name}"));
            writeln!(begin, ".pushsection {},\"xr\"", section).expect("write to String buffer is infallible");
            writeln!(begin, ".balign {align_bytes}").expect("write to String buffer is infallible");
            write_linkage(&mut begin).expect("write to String buffer is infallible");
            writeln!(begin, ".def {asm_name}").expect("write to String buffer is infallible");
            writeln!(begin, ".scl 2").expect("write to String buffer is infallible");
            writeln!(begin, ".type 32").expect("write to String buffer is infallible");
            writeln!(begin, ".endef").expect("write to String buffer is infallible");
            writeln!(begin, "{asm_name}:").expect("write to String buffer is infallible");
        ...
        BinaryFormat::Wasm => {
            ...
            writeln!(begin, ".section {section},\"\",@").expect("write to String buffer is infallible");
            // wasm functions cannot be aligned, so skip
            write_linkage(&mut begin).expect("write to String buffer is infallible");
            if let Visibility::Hidden = visibility {
                writeln!(begin, ".hidden {asm_name}").expect("write to String buffer is infallible");
            }
            writeln!(begin, ".type {asm_name}, @function").expect("write to String buffer is infallible");
            if !arch_prefix.is_empty() {
                writeln!(begin, "{}", arch_prefix).expect("write to String buffer is infallible");
            ...
            writeln!(begin, "{asm_name}:").expect("write to String buffer is infallible");
            writeln!(begin, ".functype {asm_name} {}", wasm_functype(tcx, fn_abi)).expect("write to String buffer is infallible");

            writeln!(end).expect("write to String buffer is infallible");
            // .size is ignored for function symbols, so we can skip it
            writeln!(end, "end_function").expect("write to String buffer is infallible");
            writeln!(end, ".Lfunc_end_{asm_name}:").expect("write to String buffer is infallible");
        }
        BinaryFormat::Xcoff => {
            ...
            // NOTE: .csect not implemented in LLVM; section start is missing for this target.
            ...
            writeln!(begin, ".align {}", align_bytes).expect("write to String buffer is infallible");

            write_linkage(&mut begin).expect("write to String buffer is infallible");
            if let Visibility::Hidden = visibility {
                // NOTE: `.globl {asm_name}, hidden` syntax may be valid but not used.
                // but due to limitations with `.weak` (see above) we can't really use that in general yet
            }
            writeln!(begin, "{asm_name}:").expect("write to String buffer is infallible");

            writeln!(end).expect("write to String buffer is infallible");
            // NOTE: Section end directive not emitted; may need .previous or equivalent.
        }
    }
