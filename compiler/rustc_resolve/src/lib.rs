...
enum ResolutionError<'ra> {
    ...
    // NOTE(generic_const_parameter_types): this should give custom output specifying it's only
...
impl<'ra> PathResult<'ra> {
    fn failed(
        ...
        let (message, label, suggestion) = if finalize {
            ...
            // NOTE: this output isn't actually present in the test suite.
...
impl<'ra> DeclData<'ra> {
    ...
    // NOTE: how can we integrate this with the `update_resolution`?
...
pub struct Resolver<'ra, 'tcx> {
    ...
    /// NOTE: `derives` are missing in these parent scopes and need to be taken from elsewhere.
...
impl<'tcx> Resolver<'_, 'tcx> {
    ...
    fn create_def(
        ...
        // NOTE: remove `def_span` body, pass in the right spans here and call `tcx.at().create_def()`
...
impl<'ra, 'tcx> Resolver<'ra, 'tcx> {
    ...
    fn traits_in_module(
        ...
        for &(trait_name, trait_binding, trait_module, lint_ambiguous) in
            traits.as_ref().expect("invariant: value is present").iter()
        {
    ...
    fn record_use_inner(
        ...
        if let Some(b2) = used_decl.ambiguity.get() {
            let ambiguity_error = AmbiguityError {
                ...
                b1: used_decl,
                b2,
                scope1: Scope::ModuleGlobs(used_decl.parent_module.expect("invariant: binding has parent module"), None),
                scope2: Scope::ModuleGlobs(b2.parent_module.expect("invariant: binding has parent module"), None),
    ...
    fn resolve_crate_root(&self, ident: Ident) -> Module<'ra> {
        ...
        let mark = if ident.name == kw::DollarCrate {
            ...
            // NOTE: this is only a guess and it doesn't work correctly for `macro_rules!`
    ...
    fn disambiguate_macro_rules_vs_modularized(
        ...
        let macro_rules = macro_rules.parent_module.expect("invariant: binding has parent module");
        let modularized = modularized.parent_module.expect("invariant: binding has parent module");
    ...
    fn resolve_rustdoc_path(
        ...
        match self.cm().maybe_resolve_path(&segments, Some(ns), &parent_scope, None) {
            PathResult::Module(ModuleOrUniformRoot::Module(module)) => Some(module.res().expect("invariant: module has resolution")),
            ...
            // tRust: invariant — after handling all valid PathResult variants, remaining Module/Indeterminate cases are impossible
    ...
    fn legacy_const_generic_args(&mut self, expr: &Expr) -> Option<Vec<usize>> {
        ...
        if path.segments.last().expect("invariant: non-empty collection").args.is_some() {
            return None;
        }
...
fn build_extern_prelude<'tcx, 'ra>(
    ...
    let mut extern_prelude: FxIndexMap<IdentKey, ExternPreludeEntry<'ra>> = tcx
        ...
        .filter_map(|(name, entry)| {
            ...
            // NOTE: reject `--extern self` and similar in option parsing instead.
    ...
    loop {
        if let ModuleKind::Def(.., name) = module.kind {
            if let Some(parent) = module.parent {
                ...
                names.push(name.expect("invariant: value is present"));
                module = parent
            } else {
                break;
            }
...
// NOTE: these are cells for caches that can be populated even during speculative resolution,
...
// NOTE: `*_unchecked` methods in the module below should be eliminated in the process
