...
//
// Module-local error types (GovernorError, ReplayError, etc.) remain in their
// respective modules for internal use. RouterError provides #[from] conversions
// so callers can use a single Result<T, RouterError> at the crate boundary.
//
...
use crate::solver_execution::ExecutionError;
...
/// matching (e.g., `GovernorError::Shutdown` vs `GovernorError::AcquireTimeout`).
#[derive(Debug, Error)]
#[non_exhaustive]
pub enum RouterError {
    /// Resource governor error (timeout budget, memory limit, shutdown).
    #[error("governor: {0}")]
...
impl Z4EncodeError {
    /// Convenience constructor for z4 API errors.
    pub(crate) fn api(operation: &'static str, err: impl std::fmt::Display) -> Self {
        Z4EncodeError::Z4Api { operation, message: err.to_string() }
    }
}
...
mod tests {
    ...
    #[test]
    fn test_router_error_from_governor() {
        let gov_err = GovernorError::Shutdown;
        let router_err: RouterError = gov_err.into();
        assert!(matches!(router_err, RouterError::Governor(GovernorError::Shutdown)));
        let msg = router_err.to_string();
        assert!(msg.contains("governor"), "error message should contain 'governor': {msg}");
    }
    ...
    fn test_router_error_from_isolation() {
        ...
        assert!(matches!(router_err, RouterError::Isolation(IsolationError::Timeout { timeout_ms: 5000 })));
    }
    ...
    fn test_router_error_from_solver_process() {
        let sp_err = SolverProcessError::BinaryNotFound {
            solver: "z3",
            hint: "not in PATH".to_string(),
        };
        let router_err: RouterError = sp_err.into();
        assert!(matches!(router_err, RouterError::SolverProcess(SolverProcessError::BinaryNotFound { .. })));
    }

    #[test]
    fn test_router_error_from_z4_encode() {
        let z4_err = Z4EncodeError::UnsupportedFormula {
            description: "BitVec".to_string(),
        };
        let router_err: RouterError = z4_err.into();
        assert!(matches!(router_err, RouterError::Z4Encode(Z4EncodeError::UnsupportedFormula { .. })));
    }

    #[test]
    fn test_router_error_is_non_exhaustive() {
        // Verify RouterError can be matched with a wildcard (non_exhaustive).
        let err: RouterError = GovernorError::Shutdown.into();
        match err {
            RouterError::Governor(_) => {}
            _ => panic!("expected Governor variant"),
        }
    }
}
