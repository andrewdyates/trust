...
//     - trust-vcgen (VC generation from synthetic MIR)
//     - trust-router/SmtLibBackend (real z4 verification)
...
/// trust-vcgen, and verifies them with real z4.
///
...
impl VerifyContext for FileLoopContext {
    fn verify_vcs(
        ...
        for vc in &vcs {
            ...
            let tag = if result.is_proved() { "PROVED" } else if result.is_failed() { "FAILED" } else { "UNKNOWN" };
    ...
    fn strengthen_specs(
        ...
        eprintln!("    strengthen: {} proposals, {} failures analyzed",
            output.proposals.len(), output.failures_analyzed);
        ...
        match plan {
            Ok(plan) if !plan.is_empty() => {
                ...
                let source = std::fs::read_to_string(&self.source_path)
                    .expect("should read source file");
...
"#;
        ...
        let func = self.build_mir();
        let new_vcs = trust_vcgen::generate_vcs(&func);
        ...
        new_vcs
            .into_iter()
            .filter(|vc| failed_kinds.iter().any(|k| std::mem::discriminant(*k) == std::mem::discriminant(&vc.kind)))
            .collect()
    }
}
...
fn test_e2e_prove_strengthen_backprop_loop_converges() {
    ...
    let initial_vcs = trust_vcgen::generate_vcs(&func);
    ...
    for (i, record) in result.history.iter().enumerate() {
        eprintln!("  Iter {}: proved={} failed={} unknown={} strengthened={}",
    ...
    let final_source = std::fs::read_to_string(&source_path)
        .expect("should read final source");
    eprintln!("\n  Final source:");
    ...
    assert!(
        result.history[0].failed > 0,
        "first iteration must detect the overflow bug"
    );
...
fn test_e2e_file_context_detects_overflow() {
    ...
    assert!(
        ...
        results.iter().map(|(vc, r)| format!("{:?}->{}", vc.kind, if r.is_proved() { "P" } else if r.is_failed() { "F" } else { "U" })).collect::<Vec<_>>()
    );
...
fn test_e2e_strengthen_rewrites_source() {
    ...
    assert_ne!(
        modified, BUGGY_MIDPOINT_SOURCE,
        "source must be modified by strengthen + backprop"
    );
    assert!(
...
fn test_e2e_safe_midpoint_proves() {
    ...
    for vc in &vcs {
        ...
        let tag = if result.is_proved() { proved += 1; "PROVED" }
                  else if result.is_failed() { failed += 1; "FAILED" }
                  else { "UNKNOWN" };
    ...
    assert!(
        proved > 0,
        "z4 must prove at least some VCs for safe midpoint"
    );
...
"#;
    ...
    // Step 5: CONVERGE
    eprintln!("\n--- Step 5: CONVERGE ---");
    eprintln!("  Iteration 1: overflow detected ({} failures)", crate_results.functions[0].results.len());
