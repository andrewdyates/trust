...
src/main.rs instead of `./project/src/main.rs)`.
...
```bash
rustc --remap-path-prefix "src/=/redacted/src" --remap-path-prefix "./project=/redacted"
```

### Example

```bash
rustc --remap-path-prefix "./project=/redacted"
```

This example replaces all occurrences of `./project` in emitted paths with `/redacted`.
