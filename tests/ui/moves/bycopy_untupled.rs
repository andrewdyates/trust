// tRust: regression test for rust-lang/rust#155241 (upstream miscompilation)
// Arguments passed indirectly via a hidden pointer must be copied to an alloca,
// except for by-val or by-move. Without the fix, closures with rust-call ABI
// can corrupt the caller's data through the indirect pointer.
//
